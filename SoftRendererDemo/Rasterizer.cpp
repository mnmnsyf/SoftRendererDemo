...
void Rasterizer::clear(const Vec3f& color) {
	...
	//2. 清空深度缓冲
	// 将所有深度值初始化为正无穷大 (表示该像素目前距离摄像机无限远)
...
void Rasterizer::save_depth_to_ppm(const char* filename) {
	...
	// 1. 写入 PPM 头部 (灰度图也是用 P3 RGB 格式存，只是 R=G=B)
	ofs << "P3\n" << width << " " << height << "\n255\n";

	// 2. 第一遍遍历：寻找当前场景中 Z 的最小值和最大值
	// 我们必须忽略初始的 Infinity，只关心实际画上去的像素
	...
	// 防止除以零（如果场景是空的，或者所有像素深度都一样）
	if (empty_scene) {
		min_z = 0; max_z = 1;
	}
	else if (min_z == max_z) {
		// 只有一个平面，无法归一化，人为拉开一点范围
		max_z = min_z + 0.0001f;
	}

	float range = max_z - min_z;

	std::cout << "导出深度图: Min Z=" << min_z << " Max Z=" << max_z << std::endl;
	...
	ofs.close();
	std::cout << "深度图已保存至: " << filename << std::endl;
}